...
namespace Nop.Plugin.Misc.CustomRobotsTxt
{
    public class RobotsTxtPlugin : BasePlugin, IMiscPlugin
    {
        ...
        private void RemoveSettings()
        {
            ...
            var setting = allSettings.FirstOrDefault(s => string.Compare(s.Name, "Plugins.Misc.CustomRobotsTxt.BlogPost", true) == 0);
            if (setting != null)
            {
                _settingService.DeleteSetting(setting);
            }

            setting = allSettings.FirstOrDefault(s => string.Compare(s.Name, "Plugins.Misc.CustomRobotsTxt.Category", true) == 0);
            if (setting != null)
            {
                _settingService.DeleteSetting(setting);
            }

            setting = allSettings.FirstOrDefault(s => string.Compare(s.Name, "Plugins.Misc.CustomRobotsTxt.Manufacturer", true) == 0);
            if (setting != null)
            {
                _settingService.DeleteSetting(setting);
            }

            setting = allSettings.FirstOrDefault(s => string.Compare(s.Name, "Plugins.Misc.CustomRobotsTxt.NewsItem", true) == 0);
            if (setting != null)
            {
                _settingService.DeleteSetting(setting);
            }

            setting = allSettings.FirstOrDefault(s => string.Compare(s.Name, "Plugins.Misc.CustomRobotsTxt.Product", true) == 0);
            if (setting != null)
            {
                _settingService.DeleteSetting(setting);
            }

            setting = allSettings.FirstOrDefault(s => string.Compare(s.Name, "Plugins.Misc.CustomRobotsTxt.Topic", true) == 0);
            if (setting != null)
            {
                _settingService.DeleteSetting(setting);
            }

            setting = allSettings.FirstOrDefault(s => string.Compare(s.Name, "Plugins.Misc.CustomRobotsTxt.All", true) == 0);
            if (setting != null)
            {
                _settingService.DeleteSetting(setting);
            }

            setting = allSettings.FirstOrDefault(s => string.Compare(s.Name, "Plugins.Misc.CustomRobotsTxt.Vendor", true) == 0);
            if (setting != null)
            {
                _settingService.DeleteSetting(setting);
            }
